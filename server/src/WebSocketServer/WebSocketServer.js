...
class WebSocketServer {
  ...
  setEvents() {
    this.webSocketServer.on( 'connection', ( client, req ) => {
      ...
      client.id = uuid.v4();
      if ( clientPath === 'photo_capture' && !this.photoCaptureClient) this.setPhotoCaptureClient( client );
      else if ( clientPath === 'sfm' && !this.SfMClient) this.setSfMClient( client );
  ...
  setPhotoCaptureClient( client ) {
    client.on( 'message', ( message ) => {
      if (message.status) this.pcStatus = message.status
  ...
  setSfMClient( client ) {
    client.on( 'message' , ( message ) => {
      if (message.status) this.SfMStatus = message.status
      console.log( 'received: %s', message );
    } );
    client.on('close', () => {
